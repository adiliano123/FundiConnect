...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .hero { background: linear-gradient(135deg, #1C9AD6, #0e7ab5); padding: 28px 32px; text-align: center; }
  ...
  .badge-customer { display: inline-block; background: #3b82f6; color: #fff; font-size: 12px; font-weight: 700; padding: 3px 10px; border-radius: 20px; }
  .badge-technician { display: inline-block; background: #f59e0b; color: #fff; font-size: 12px; font-weight: 700; padding: 3px 10px; border-radius: 20px; }
  .btn { display: block; width: fit-content; margin: 24px auto 0; padding: 14px 36px; border-radius: 8px; text-decoration: none; font-weight: 700; font-size: 15px; color: #ffffff; background: #1D234F; }
...
<div class="wrapper">
  <div class="card">
    ...
    <div class="hero">
      ...
      <p>A new {{ $newUser->role }} has joined FundiConnect</p>
    </div>
    <div class="body">
      ...
      <p style="font-size:14px;line-height:1.7;color:#4b5563;margin-top:12px;">
        A new user has just registered on the FundiConnect platform. Here are their details:
      </p>
      ...
      <div class="detail-box">
        ...
        <div class="detail-row">
          ...
          <span class="detail-value">{{ $newUser->phone ?? 'Not provided' }}</span>
        </div>
        <div class="detail-row">
          <span class="detail-label">Role</span>
          <span class="detail-value">
            <span class="badge-{{ $newUser->role }}">{{ ucfirst($newUser->role) }}</span>
          </span>
        </div>
        <div class="detail-row">
          <span class="detail-label">Registered At</span>
          <span class="detail-value">{{ $newUser->created_at->format('D, d M Y \a\t H:i') }}</span>
        </div>
      </div>

      @if($newUser->role === 'technician')
        <p style="font-size:14px;color:#f59e0b;background:#fffbeb;border:1px solid #fde68a;border-radius:8px;padding:12px 16px;margin-top:4px;">
          ⚠️ This user registered as a <strong>technician</strong> and requires verification before they can receive bookings.
        </p>
      @endif
