    ...
    <div className="space-y-5">
      {payingBooking && (
        <PaymentModal
          ...
          onSuccess={() => { setPayingBooking(null); fetch(); }}
        />
      )}
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <Link href="/technicians" className="inline-flex items-center gap-2 bg-[#1D234F] text-white text-sm font-semibold px-4 py-2.5 rounded-xl hover:bg-[#161b3d] transition-colors">
          + New Booking
        </Link>
      </div>
      ...
      <Card>
        {loading ? (
          ...
          <EmptyState icon={BookOpen} title="No bookings found" description="You haven't made any bookings yet." />
        ) : (
                  ...
                  <tr key={b.id} className="hover:bg-gray-50/70 transition-colors">
                      ...
                      {b.payment ? (
                        <div>
                          <span className={`px-2.5 py-1 rounded-full text-xs font-semibold ${
                            b.payment.status === 'completed' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'
                          }`}>
                            {b.payment.status === 'completed' ? '✓ Paid' : 'Pending'}
                          </span>
                          <p className="text-xs text-gray-500 mt-1">TZS {b.payment.amount.toLocaleString()}</p>
                        </div>
                      ) : (
                    ...
                    <td className="py-3">
                      <div className="flex gap-2">
                        ...
                        {['accepted', 'awaiting_payment', 'in_progress', 'completed'].includes(b.status) && !b.payment?.status?.match(/completed/) && (
                          <button
